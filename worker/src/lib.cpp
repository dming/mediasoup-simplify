  ...
  PayloadChannelWriteCtx payloadChannelWriteCtx)
{
	...
	MS_DEBUG_TAG(info, "[dming] starting mediasoup-worker process [version:%s]", version);
	...
	try
	{
		...
		IgnoreSignals();
#endif
		MS_DEBUG_TAG(info, "[dming] After ClassInit and Before create worker.");
		// Run the Worker.
		Worker worker(channel.get(), payloadChannel.get());

		MS_DEBUG_TAG(info, "[dming] After create worker and Before ClassDestroy.");
		// Free static stuff.
		DepLibUV::ClassDestroy();
		...
		MS_DEBUG_TAG(info, "[dming] After ClassDestroy and Return 0.");
		return 0;
	}
	catch (const MediaSoupError& error)
	{
